...
void publishResult(){
    ...
    else
    {
        ...
        // 保存结果的精度
        foutC.precision(6);
        foutC << lidarOdometry.header.stamp << " "
              << lidarOdometry.pose.pose.position.x << " "
              << lidarOdometry.pose.pose.position.y << " "
              << lidarOdometry.pose.pose.position.z << " "
              << lidarOdometry.pose.pose.orientation.x << " "
              << lidarOdometry.pose.pose.orientation.y << " "
              << lidarOdometry.pose.pose.orientation.z << " "
              << lidarOdometry.pose.pose.orientation.w << std::endl;
        foutC.close();
    }
    //发布优化后的轨迹
...
//    nh.getParam("RESULT_PATH", RESULT_PATH);
    // 读取参数
    // 如果参数服务器中参数"RESULT_PATH"存在， 把"RESULT_PATH"的值传递给变量RESULT_PATH；如果"RESULT_PATH"不存在，就传递默认值"hahaha"
    nh.param<std::string>("RESULT_PATH",RESULT_PATH,"hahaha");
    ROS_INFO("RESULT_PATH is [%s]", RESULT_PATH.c_str());
