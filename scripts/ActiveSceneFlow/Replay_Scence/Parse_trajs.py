...
def main():
    ...
    parser.add_argument('--log_files', type=str, nargs='+', default='./logfile-Town02Random.txt')
    ...
    for i, log_file in enumerate([args.log_files]):
        ...
        #                     r"number_of_wheels = 4[\s\S]+"
        # a = "Create 246: vehicle.dodge.charger_police_2020 (1) at (18970.3, 30747.5, 26.8807)\n  generation = 2\n  number_of_wheels = 4\n  sticky_control = true\n  object_type = \n  color = 0,0,0\n  role_name = autopilot\n Create 247: vehicle.toyota.prius (1) at (19369.4, 21327.3, 25.6207)\n  generation = 1\n  number_of_wheels = 4\n  sticky_control = true\n  object_type = \n  color = 227,227,227\n  role_name = autopilot\n"
        vehicle_pattern2 = r"Create (?P<id>\d+): (?P<name>[vehicle]+.\w+.\w+) [(](?P<att_id>\d+)[)] at [(](?P<x>[+-]?\d+.\d+|[+-]\d+.\d+e[+-][0-9]+|[+-]\d+), (?P<y>[+-]?\d+.\d+|[+-]\d+.\d+e[+-][0-9]+|[+-]\d+), (?P<z>[+-]?\d+.\d+|[+-]?\d+.\d+e[+-][0-9]+|[+-]?\d+)[)\n]+"\
                            r"(.|[generation = \d+\n]+)number_of_wheels = 4"      
        # vehicle_pattern2 = r"Create (?P<id>\d+): (?P<name>[vehicle]+.\w+.\w+) [(](?P<att_id>\d+)[)] at [(](?P<x>[+-]?\d+.\d+|[+-]\d+.\d+e[+-][0-9]+|[+-]\d+), (?P<y>[+-]?\d+.\d+|[+-]\d+.\d+e[+-][0-9]+|[+-]\d+), (?P<z>[+-]?\d+.\d+|[+-]?\d+.\d+e[+-][0-9]+|[+-]?\d+)[)\n]+"\
        #                     r"(?P<nonsense>[\s\S]+|[number_of_wheels = 4][\n])"
        ...
        for i_c in range(0,len(contents)-1,1):  
            print(i_c)
        ...
        print(v)
        height = len(frame_vehicles) #Frame Number
        depth = 8 # id, frame_id, x,y,z,rx,py,yz
        cmd_arr = np.full([height, width, depth], np.nan) #Frame Number, Vehicles Number, Transform Infos
