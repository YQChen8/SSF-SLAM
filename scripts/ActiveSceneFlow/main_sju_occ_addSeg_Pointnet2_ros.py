...
from utils.datasets.my_carla import Batch
...
def _init_(args):
    if args.rm_history and not args.eval:
        ...
        if not os.path.exists(args.model_dir + args.exp_name + '/' + 'models'):
            os.makedirs(args.model_dir + args.exmain_sju_occ_rosp_name + '/' + 'models')
...
def test_one_epoch(args, net, test_loader, epoch=0):
    ...
    '''HYPER PARAMETER'''
    experiment_dir = '/home/zhijun/catkin_ws/src/SSF-SLAM/scripts/ActiveSceneFlow/Pointnet_Pointnet2_pytorch-master/log/sem_seg/' + args.log_dir
    ...
    sys.path.append('//scripts/ActiveSceneFlow/Pointnet_Pointnet2_pytorch-master/models')
    ...
    with tqdm(enumerate(test_loader), total=len(test_loader), smoothing=0.9) as pbar:
        for i, data in enumerate(test_loader):
            ...
            flow_gt_seg = data['mask'][0].flatten().numpy()  # 12032881 个是场景流的Seg真值
            # print(flow_gt_seg.shape)
            # print(type(flow_gt_seg))
            # exit()
            ...
            if use_publish_ros:
                np_src = pc1.permute(0, 2, 1).squeeze(0).cpu().numpy()  # 这个是点云
                np_flow = pred_flow_sum.permute(0, 2, 1).cpu().detach().squeeze(0).numpy()  # 这个是网络算出来的场景流
                flow_gt = flow.permute(0, 2, 1).squeeze(0).cpu().numpy()  # 这个是场景流的真值
                ...
                if len(np_src.shape) == 3:
                    msg.height = np_src.shape[1]
                    msg.width = np_src.shape[0]
                else:
                    msg.height = 1
                    msg.width = len(np_src)
                ...
                msg.point_step = 12
                msg.row_step = msg.point_step * np_src.shape[0]
                msg.is_dense = False
                msg.data = np.asarray(np_src, np.float32).tobytes()

                velodyne_points_pub.publish(msg)

                ################################################################ 发布点云、里程计、轨迹、地图

                # move_gt = data['ground_truth'][1].squeeze(1)
                # move_gt_seg = data['mask'][0].squeeze(1)  # 12032881
                # np_flow = np_flow.data.numpy()
                # np_flow_temp = np.concatenate([np_flow, np_flow, np_flow], axis=1)
                temp = np.pad(np_flow, ((0, 0), (0, 3)), 'constant')
                np_flow_temp = np.concatenate([np_src, temp], axis=1) # 点云+场景流一起喂进去分类
                # np_flow_temp = np.pad(np_src, ((0, 0), (0, 6)), 'constant')
                np_flow_temp = torch.Tensor(np_flow_temp).unsqueeze(0)
                np_flow_temp= np_flow_temp.float().cuda()
                np_flow_temp = np_flow_temp.transpose(2, 1)

                seg_pred, trans_feat = classifier(np_flow_temp)
                pred_val = seg_pred.contiguous().cpu().data.numpy()
                pred_seg = np.argmax(pred_val, 2).flatten()

                bg_index = np.argwhere(pred_seg == 0).flatten()  # 找出背景label所在的索引

                target = np_src[bg_index] + np_flow[bg_index]
                source = np_src[bg_index]
                ...
                if use_visual:
                    ...
                    map_size = {0: 0.5, 1: 5}
                    ########################################同时画一下聚类分类的结果##################################
                    GM_move_gt = np.concatenate((np_flow, np_src), axis=1)
                    GM_model = GaussianMixture(n_components=2)
                    # print(move_dis.shape)
                    # 调用 fit 方法对数据进行聚类，先用高斯模糊算法，感觉效果比较好
                    GM_all_label = GM_model.fit_predict(GM_move_gt)  # 这个是所有分类
                    # print(Counter(all_label))
                    GM_bg_label = Counter(GM_all_label).most_common(1)[0][0]  # 找出出现最多的label作为背景label
                    # print('Most label\n', bg_label)  # 调用Counter函数
                    GM_bg_label_index = np.argwhere(GM_all_label == GM_bg_label).flatten()  # 找出背景label所在的索引
                    # print(bg_label_index.shape)
                    # GM_bg_label_index = GM_bg_label_index[0: -1:10]
                    # print(bg_label_index.shape)
                    # exit()
                    # print(bg_index)
                    GM_all_label = np.ones_like(GM_all_label)  # 更新所有分类数组
                    GM_all_label[GM_bg_label_index] = 0  # 把背景直接二分类，背景标为0
                    color = list(map(lambda x: map_color[x], GM_all_label))
                    size = list(map(lambda x: map_size[x], GM_all_label))
                    ax[0][0].set_xlim(-100, 100)
                    ax[0][0].set_ylim(-100, 100)
                    ax[1][0].set_xlim(-20, 20)
                    ax[1][0].set_ylim(-20, 20)
                    ax[0][0].scatter(np_src[:, 0], np_src[:, 1], c=color, s=size)
                    ax[1][0].scatter(np_flow[:, 0], np_flow[:, 1], c=color, s=size)  # 绘制原始数据点，颜色按照聚类标签区分

                    ########################################下面是网络分类##################################
                    color = list(map(lambda x: map_color[x], pred_seg))
                    size = list(map(lambda x: map_size[x], pred_seg))
                    ax[0][1].set_xlim(-100, 100)
                    ax[0][1].set_ylim(-100, 100)
                    ax[1][1].set_xlim(-20, 20)
                    ax[1][1].set_ylim(-20, 20)
                    ax[0][1].scatter(np_src[:, 0], np_src[:, 1], c=color, s=size)
                    ax[1][1].scatter(np_flow[:, 0], np_flow[:, 1], c=color, s=size)  # 绘制原始数据点，颜色按照聚类标签区分
                    # ax.scatter3D(points[:, 0], points[:, 1], points[:, 2], c=kmeans.labels_)  # 绘制原始数据点，颜色按照聚类标签区分
                    # # ax.scatter3D(move_gt[:, 0], move_gt[:, 1], move_gt[:, 2], c=color)  # 绘制原始数据点，颜色按照聚类标签区分
                    # plt.subplot(211)
                    # plt.show()  # 显示图像
                    # my_accuracy_score(move_gt_seg, all_label)
                    # my_accuracy_score(move_gt_seg, all_label_test)
                    # exit()
                    # exit()python plt.scatter 透明度
                    color = list(map(lambda x: map_color[x], flow_gt_seg))
                    size = list(map(lambda x: map_size[x], flow_gt_seg))
                    # color = list(map(lambda x: map_color[x], move_gt_seg))
                    ax[0][2].set_xlim(-100, 100)
                    ax[0][2].set_ylim(-100, 100)
                    ax[1][2].set_xlim(-20, 20)
                    ax[1][2].set_ylim(-20, 20)
                    ax[0][2].scatter(np_src[:, 0], np_src[:, 1], c=color, s=size)
                    ax[1][2].scatter(np_flow[:, 0], np_flow[:, 1], c=color, s=size)  # 绘制原始数据点，颜色按照聚类标签区分
                    # plt.subplot(212)
                    fig.tight_layout()
                    plt.savefig('plt_results/00_8192_100_GPG/' + str(i) + '.jpg')
                    plt.close('all') #关闭传入的 figure 对象
...
def main():
    ...
    if rospy.has_param('~DATASET_PATH'):  # 是否存在参数
        ...
        root_dir = "/home/zhijun/ISUS/public_dataset_nas/carla_scene_flow2/train/record2023_0410_1057/rm_road/SF/00"
    ...
    parser.add_argument('--model_path', type=str,
                        default='/home/zhijun/catkin_ws/src/SSF-SLAM/scripts/ActiveSceneFlow/checkpoints/TFlow_titan4_8192_100_new/models',
                        metavar='N',
                        help='Pretrained model path')
    ...
    ####################12032881 添加了分类模型的#############################################################
    parser.add_argument('--seg_model', type=str, default='my_pointnet2_sem_seg',
                        help='model name [default: pointnet_sem_seg]')
    parser.add_argument('--log_dir', type=str, default='my_pointnet2_sem_seg', help='Log path [default: None]')
    ...
    # torch.autograd.set_detect_anomaly(True)f
