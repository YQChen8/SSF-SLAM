...
def find_Ad_vehicles_location(vehicles,labels):
    ...
    # return {}
    # print(labels)
    # a = "vehicle.carlamotors.carlacola_1203_2020"
    v_pattern = r"vehicle.(?P<nonsense>[\s\S]+[_])(?P<v_id>\d+)"
    # content = re.findall(v_pattern, a)
    for vehicle in vehicles:
        content = re.findall(v_pattern, vehicle)
        v_id = content[0][1]
        print(v_id)
        lidar_location,lidar_rotation,tmp_rotation = get_sensor_transform(v_id,labels,'lidar')
        camera_location,camera_rotation,id = get_sensor_transform(v_id,labels,'camera')[0]
...
def FileCompletetionValidate(path, vehicles, total_num):
    raw_data_file = os.listdir(path)
    # vehicles =  vehicles
    print("Start file validation.\n.")
    print("Total number of files is %d\n."%(total_num))
    for item in vehicles:
        sub_path = os.path.join(path, item)
        sub_sub_dirs = os.listdir(sub_path)
    
        for sub_item in sub_sub_dirs:
            sub_item = os.path.join(sub_path, sub_item)
            pattern_txt = glob.glob(os.path.join(sub_item, '*.txt'))
            pattern_png = glob.glob(os.path.join(sub_item, '*.png'))
            pattern_bin = glob.glob(os.path.join(sub_item, '*.bin'))
            print(sub_item + " has %d text files, %d png files, and %d bin file, repectively."%(len(pattern_txt),len(pattern_png),len(pattern_bin)))
            # datapath = glob.glob(pattern_txt)
            # if len(datapath) == total_num:
            #     print(sub_item + "has " + str(len(datapath)) + " files.")
            # else:
            #     print(sub_item + "lacks " + str(total_num - len(datapath)) + " files.")


if __name__ == "__main__":
    raw_data_path = 'tmp/record2022_0120_1056'
    raw_data_path = 'tmp/record2022_0120_1043'
    ...
    for _frame in frames[frame_start:frame_end:frame_hz]:
        ...
        car_id = [1219, 1203, 1209, 1200, 1211, 1202, 1195, 1214, 1215]
        car_id = [1045]
        tmp_car_id = []
        for item in car_id:
            tmp_car_id += [str(item)]
        ...
        for ego_vehicle_label in labels:
            ...
            if 'vehicle' in ego_vehicle_label[0]:# and tmp_name in AD_vehicles_location.keys():# and '319' in ego_vehicle_label[1]:
                ...
                for index, camera in enumerate(camera_info_list):
                    ...
                    img_path = write_labels(raw_data_path, _frame, ego_vehicle_label, tmp_labels, pointCloud, 0, camera_id, calib_info_list, use_seg=False)
                    # print(label2d)
                    show_flag = False
                    if show_flag:
    ...
    FileCompletetionValidate('dataset/'+raw_data_path[4:], AD_vehicles, cnt)
