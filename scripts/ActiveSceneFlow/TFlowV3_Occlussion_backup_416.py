...
class RefineFlowRegressor(nn.Module):
    def __init__(self, nsample=8, in_channel=128, feat_channel=128, voxel_size=0.25, resolution=6, mlp=[128, 128, 128], feat_mlp=[128, 128], flow_mlp=[128, 128], iter_num=1, use_flow=True, use_flow_feats=True):
        ...
        self.cost = PointConvTransFlowV2(nsample, in_channel, feat_channel, mlp, flow_mlp, voxel_size=voxel_size, resolution=resolution, use_flow=use_flow)
        # warping
...
class TFlow(nn.Module):
    def __init__(self, npoint=8192, use_leaky=True):
        ...
        self.flow3_r = RefineFlowRegressor(nsample=16, in_channel=256, feat_channel=0, voxel_size=0.4, resolution=3,  mlp=[256, 256], feat_mlp=[256, 256], flow_mlp=[128, 128], use_flow=False, use_flow_feats=False)

        self.su2 = PointNetSetUpConv(nsample=16, radius=2.4, f1_channel=128, f2_channel=256, mlp=[128, 128], mlp2=[128,128])
        self.flow2_r = RefineFlowRegressor(nsample=16, in_channel=128+64, feat_channel=128, voxel_size=0.25, resolution=3,  mlp=[128, 128], flow_mlp=[128, 128], use_flow=True)

        self.su1 = PointNetSetUpConv(nsample=16, radius=2.4, f1_channel=64, f2_channel=128, mlp=[64, 64], mlp2=[64,64])
        self.flow1_r = RefineFlowRegressor(nsample=16, in_channel=64+32, feat_channel=128, voxel_size=0.15, resolution=3,  iter_num=1, mlp=[64, 64], feat_mlp=[64, 64], flow_mlp=[64, 64], use_flow=True)
        
        self.su0 = PointNetSetUpConv(nsample=16, radius=2.4, f1_channel=32, f2_channel=64, mlp=[64, 64], mlp2=[64,64])
        self.flow0_r = RefineFlowRegressor(nsample=16, in_channel=64+32, feat_channel=64, voxel_size=0.15, resolution=3,  iter_num=3, mlp=[64, 64], feat_mlp=[64, 64], flow_mlp=[64, 64], use_flow=True)
