...
class RefineFlowRegressor(nn.Module):
    def __init__(self, nsample=8, in_channel=128, feat_channel=128, mlp=[128, 128, 128], flow_mlp=[128, 128],
                 use_flow=True):
...
class TFlow(nn.Module):
    def __init__(self, npoint=8192):
        ...
        self.su3 = PointNetSetUpConv(nsample=16, radius=2.4, f1_channel=256, f2_channel=512, mlp=[256, 256],
                                     mlp2=[256, 256])
        self.flow3_r = RefineFlowRegressor(nsample=16, in_channel=256, feat_channel=0, mlp=[256, 256],
                                           flow_mlp=[128, 128], use_flow=False)

        self.su2 = PointNetSetUpConv(nsample=16, radius=2.4, f1_channel=128, f2_channel=256, mlp=[128, 128],
                                     mlp2=[128, 128])
        self.flow2_r = RefineFlowRegressor(nsample=16, in_channel=128 + 64, feat_channel=128, mlp=[128, 128],
                                           flow_mlp=[128, 128], use_flow=True)

        self.su1 = PointNetSetUpConv(nsample=16, radius=2.4, f1_channel=64, f2_channel=128, mlp=[64, 64], mlp2=[64, 64])
        self.flow1_r = RefineFlowRegressor(nsample=16, in_channel=64 + 32, feat_channel=128, mlp=[64, 64],
                                           flow_mlp=[64, 64], use_flow=True)

        self.su0 = PointNetSetUpConv(nsample=16, radius=2.4, f1_channel=32, f2_channel=64, mlp=[64, 64], mlp2=[64, 64])
        self.flow0_r = RefineFlowRegressor(nsample=16, in_channel=64 + 32, feat_channel=64, mlp=[64, 64],
                                           flow_mlp=[64, 64], use_flow=True)
    ...
    def forward(self, pc1, pc2, feats1=None, feats2=None):
        ...
        c_feat_fwd_l2, c_feat_bwd_l2, l2_feats_sf, l2_flow = self.flow2_r(l2_pc1, l2_pc2, c_feat_fwd_l3_2,
                                                                          c_feat_bwd_l3_2, 5, l2_coarse_sf,
                                                                          l2_3_feats_sf)
        ...
        c_feat_fwd_l1, c_feat_bwd_l1, l1_feats_sf, l1_flow = self.flow1_r(l1_pc1, l1_pc2, c_feat_fwd_l2_1,
                                                                          c_feat_bwd_l2_1, 7, l1_coarse_sf,
                                                                          l1_2_feats_sf)
        ...
        # move_index = torch.where(flow >= 50)
        # flow[move_index] = 50
        # print(move_index)
        # print(flow.size())
        # exit()
...
def multiScaleLoss(pred_flows, gt_flow, mask, fps_idxs, alpha=[0.02, 0.04, 0.08, 0.16, 0.24]):
    ...
    for i in range(num_scale):  # 12032881
        # move_index = torch.where(gt_masks[i] == 1)
        # bg_index = torch.where(gt_masks[i] == 0)
        # gt_flows[i][move_index] = 50
        # gt_masks[i][bg_index] = 1
        # print(111111)
        # print(pred_flows[i].permute(0, 2, 1).size())
        # print(gt_flows[i].size())
        # print(gt_masks[i].size())
