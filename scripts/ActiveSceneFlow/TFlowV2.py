...
from utils.soflow import PointConvTransFlowV2, PointConvTransFlowV3, PointWarping, UpsampleFlow, \
    SceneFlowEstimatorProbPointConv, SceneFlowEstimatorProbPointConvV2  # , PointNetSetAbstraction
...
class RefineFlowRegressor(nn.Module):
    def __init__(self, nsample=8, in_channel=128, feat_channel=128, voxel_size=0.25, resolution=6, mlp=[128, 128, 128], feat_mlp=[128, 128], flow_mlp=[128, 128], use_flow=True, use_flow_feats=True):
        ...
        self.flow = SceneFlowEstimatorProbPointConv(feat_ch=feat_channel, cost_ch=mlp[-1], channels=feat_mlp, mlp=flow_mlp, use_flow=use_flow, use_flow_feats=use_flow_feats)
        self.deconv = Conv1d(feat_channel, feat_channel)
    ...
    def forward(self, pc1, pc2, feats1, feats2, c_flow=None, flow_feats=None):
        ...
        if flow_feats==None:
            ...
            flow_feats = self.deconv(flow_feats)
...
class TFlow(nn.Module):
    def __init__(self, npoint=8192, use_leaky=True):
        ...
        self.sa1 = PointNetSetAbstraction(npoint=2048, radius=0.5, nsample=32, in_channel=32, mlp=[32, 32, 64],
                                          group_all=False)
        self.sa2 = PointNetSetAbstraction(npoint=1024, radius=1.0, nsample=16, in_channel=64, mlp=[64, 64, 128],
                                          group_all=False)
        self.sa3 = PointNetSetAbstraction(npoint=256, radius=2.0, nsample=16, in_channel=128, mlp=[128, 128, 256],
                                          group_all=False)
        self.sa4 = PointNetSetAbstraction(npoint=64, radius=4.0, nsample=16, in_channel=256, mlp=[256, 256, 512],
                                          group_all=False)

        self.flow4_r = RefineFlowRegressor(nsample=16, in_channel=512, feat_channel=512, voxel_size=0.5, resolution=3, mlp=[256, 256, 256], feat_mlp=[256, 256], flow_mlp=[256, 256], use_flow=False, use_flow_feats=False)

        self.su3 = PointNetSetUpConv(nsample=16, radius=2.4, f1_channel=256, f2_channel=512, mlp=[256, 256], mlp2=[256,256])
        self.flow3_r = RefineFlowRegressor(nsample=16, in_channel=256, feat_channel=256, voxel_size=0.4, resolution=3,  mlp=[256, 256, 256], feat_mlp=[256, 256], flow_mlp=[256, 256])

        self.su2 = PointNetSetUpConv(nsample=16, radius=2.4, f1_channel=128, f2_channel=256, mlp=[128, 128], mlp2=[128,128])
        self.flow2_r = RefineFlowRegressor(nsample=16, in_channel=128, feat_channel=256, voxel_size=0.25, resolution=3,  mlp=[256, 128, 128])

        self.su1 = PointNetSetUpConv(nsample=16, radius=2.4, f1_channel=64, f2_channel=128, mlp=[128, 128], mlp2=[128,128])
        self.flow1_r = RefineFlowRegressor(nsample=16, in_channel=128, feat_channel=128, voxel_size=0.15, resolution=3,  mlp=[256, 128, 128])
    ...
    def forward(self, pc1, pc2):
        ...
        l3_feats, l3_flow = self.flow3_r(l3_pc1, l3_pc2, l3_4_feats1, l3_4_feats2, l3_coarse_sf, l3_4_feats_sf)
        ...
        l2_feats_sf, l2_flow = self.flow2_r(l2_pc1, l2_pc2, l2_3_feats1, l2_3_feats2, l2_coarse_sf, l2_3_feats_sf)
        ...
        _, l1_flow = self.flow1_r(l1_pc1, l1_pc2, l1_2_feats1, l1_2_feats2, l1_coarse_sf, l1_2_feats_sf)
