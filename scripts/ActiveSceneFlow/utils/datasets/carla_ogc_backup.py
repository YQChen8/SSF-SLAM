...
from OGC.utils.data_util import augment_transform, compress_label_id, batch_segm_to_mask

multi_file = True  # 是否是多个文件 12032881
add_Seg_after_FLow = False  # 是否融合语义信息到ground truth 12032881
add_Seg_after_PC = True # 是否融合语义信息到点云后进行计算 12032881
    ...
    def load_sequence(self, idx):
        ...
        with np.load(self.filenames[idx]) as data:
            ...
            if "s_fg_mask" in data and "t_fg_mask" in data:
                ...
                # if add_Seg_after_PC:  # 12032881
                    # print(111111111111111111111111111)
                    # print(type(data["pos1"]))
                    # print(type(data["s_fg_mask"]))
                    # mask[0] = np.concatenate((data["pos1"], data["s_fg_mask"][:, np.newaxis]), axis=1)
                    # mask[1] = np.concatenate((data["pos2"], data["t_fg_mask"][:, np.newaxis]), axis=1)
                if use_OGC:
                    pcs, segms, flows = np.stack(sequence, 0), np.stack(mask, 0), np.stack([ground_truth[1],-ground_truth[1]], 0)
                    ...
                    if self.onehot_label:
                        return pcs.astype(np.float32), segms.astype(np.float32), flows.astype(
                            np.float32), valids.astype(np.float32)
                    else:
                        return pcs.astype(np.float32), segms.astype(np.int32), flows.astype(np.float32), valids.astype(
                            np.float32)
            else:
                mask = []
