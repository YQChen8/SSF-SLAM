...
from utils.datasets.carla import Batch, add_Seg_after_FLow
    ...
    with tqdm(enumerate(test_loader), total=len(test_loader), smoothing=0.9) as pbar:
        for i, data in enumerate(test_loader):
            ...
            if use_publish_ros:
                ...
                ################################################################ 发布点云、里程计、轨迹、地图
                # bg_index = np.argwhere(abs(move_gt) > 0.01).flatten()  # 找出背景label所在的索引
                # # print(move_gt)
                # print(len(move_gt))
                # print(len(bg_index))
                #########################################################################上面是将动态点设置为固定值的代码
                # 获取里程计算R，t并发布
                # bg_index = np.argwhere(abs(move_gt[:, 3]) < 0.01).flatten()  # 找出背景label所在的索引
                # bg_index = np.argwhere(gt[:, 3] == 0).flatten()
                bg_index = np.argwhere(move_gt_seg == 0).flatten()
                # print(bg_index.shape)
                # print(bg_index_gt.shape)
                # print(set(bg_index)-set(bg_index_gt))
                # exit()
                # print(move_gt[bg_index][:, 0:3].shape)
                # print(points[bg_index].shape)

                target = points[bg_index] + move_gt[bg_index][:, 0:3]
...
def main():
    ...
    if rospy.has_param('~DATASET_PATH'):  # 是否存在参数
        ...
        root_dir = "/home/zhijun/ISUS/public_dataset_nas/carla_scene_flow2/train/record2023_0308_1439/rm_road/SF/00"
